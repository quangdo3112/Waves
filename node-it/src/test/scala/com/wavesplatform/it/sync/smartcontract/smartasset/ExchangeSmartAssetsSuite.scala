...
class ExchangeSmartAssetsSuite extends BaseTransactionSuite with CancelAfterFailure with NTPTime {
  ...
  test("use all functions from RIDE for asset script") {
    val script1 = Some(ScriptCompiler(cryptoContextScript(false), isAssetScript = true, ScriptEstimatorV2.apply).explicitGet()._1.bytes.value.base64)
    val script2 = Some(ScriptCompiler(pureContextScript(dtx, false), isAssetScript = true, ScriptEstimatorV2.apply).explicitGet()._1.bytes.value.base64)
    val script3 = Some(ScriptCompiler(wavesContextScript(dtx, false), isAssetScript = true, ScriptEstimatorV2.apply).explicitGet()._1.bytes.value.base64)
