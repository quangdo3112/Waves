...
class InvokeScriptPayAndTransferSameAssetSuite extends BaseTransactionSuite with CancelAfterFailure {
  ...
  test("_issue and transfer asset") {
    ...
    val script = Some(ScriptCompiler.compile("true", ScriptEstimatorV2.apply).explicitGet()._1.bytes.value.base64)
    ...
    val smartScript = Some(ScriptCompiler.compile(scriptText, ScriptEstimatorV2.apply).explicitGet()._1.bytes.value.base64)
