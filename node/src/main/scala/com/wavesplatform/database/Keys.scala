...
object Keys {
  ...
  def addressScript(addressId: BigInt, estimator: ScriptEstimator)(height: Int): Key[Option[Script]] =
    Key.opt(
      "address-script",
      hAddr(28, height, addressId),
      ScriptReader.fromBytes(_, estimator).explicitGet(),
      _.bytes().arr
    )
  ...
  def assetScript(asset: IssuedAsset, estimator: ScriptEstimator)(height: Int): Key[Option[Script]] =
    Key.opt(
      "asset-script",
  ...
  def transactionAt(height: Height, n: TxNum, estimator: ScriptEstimator): Key[Option[Transaction]] =
    Key.opt[Transaction](
      "nth-transaction-info-at-height",
      hNum(TransactionInfoPrefix, height, n),
      data => TransactionParsers.parseBytes(data, estimator).get,
      _.bytes()
    )
