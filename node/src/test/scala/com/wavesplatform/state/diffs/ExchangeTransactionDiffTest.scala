...
class ExchangeTransactionDiffTest extends PropSpec with PropertyChecks with Matchers with TransactionGen with Inside with NoShrink {
  ...
  property("Disable use OrderV1 on SmartAccount") {
    ...
    val script           = "true"
    val txScriptCompiled = ScriptCompiler(script, isAssetScript = false, ScriptEstimatorV2.apply).explicitGet()._1

    val sellerScript = Some(ScriptCompiler(script, isAssetScript = false, ScriptEstimatorV2.apply).explicitGet()._1)
    val buyerScript  = Some(ScriptCompiler(script, isAssetScript = false, ScriptEstimatorV2.apply).explicitGet()._1)
  ...
  def smartTradePreconditions(buyerScriptSrc: Gen[String],
    ...
    for {
      txScript <- txScript
      txScriptCompiled = ScriptCompiler(txScript, isAssetScript = false, ScriptEstimatorV2.apply).explicitGet()._1
      sellerScriptSrc <- sellerScriptSrc
      sellerScript = Some(ScriptCompiler(sellerScriptSrc, isAssetScript = false, ScriptEstimatorV2.apply).explicitGet()._1)
      buyerScriptSrc <- buyerScriptSrc
      buyerScript = Some(ScriptCompiler(buyerScriptSrc, isAssetScript = false, ScriptEstimatorV2.apply).explicitGet()._1)
